...
(function ($){
    ...
    $.fn.packDatetime = function (options){
        ...
        var defaults = {
            ...
            dateType: 'defaule'   //日期类型：1、默认 defaule 2、下一年 nextYear 3、下一月 nextMonth 4、一组日期为同一天内 sameDay
        }
        ...
        return this.datetimepicker(defaults).on('changeDate', function(ev){
            ...
            var currentHours = chooseDate.getHours();
            var currentMinutes = chooseDate.getMinutes();
            ...
            $(this).parents(".datetime-group").find('div.date').each(function( index, element) {
                ...
                if($(currentDom).attr('data-group')=="date-begin" && $(element).attr('data-group')=="date-end"  ){
                    ...
                    //开始日期变更 调整结束日期范围
                    if(defaults.dateType==='sameDay'){
                        ev.date ? $(element).datetimepicker('setStartDate',currentYear+'-'+currentMonth+'-'+currentDate+" "+currentHours+":"+currentMinutes).datetimepicker("setEndDate",currentYear+'-'+currentMonth+'-'+currentDate+" "+" 23:55") : $(element).datetimepicker('setStartDate',null).datetimepicker("setEndDate",null)
                    }else{
                    ...
                    //结束日期变更 调整开始日期范围
                    if(defaults.dateType==='sameDay'){
                        ev.date ? $(element).datetimepicker('setEndDate',currentYear+'-'+currentMonth+'-'+currentDate+" "+currentHours+":"+currentMinutes).datetimepicker("setStartDate",currentYear+'-'+currentMonth+'-'+currentDate+" "+" 00:00") : $(element).datetimepicker('setStartDate',null).datetimepicker("setEndDate",null)
                    }else{
